...
public final class FlowCore {
    ...
    public static SyncResult sync(Context c)throws Exception{
        ...
        try{
            ...
            stage="META80 최근30 고정 최고1픽"; sp.edit().putString(K_LAST_STAGE,stage).apply(); Analysis a=analyze(merged);
    ...
    public static Analysis analyze(List<Result> desc){
        ...
        // 30FIX 핵심: 추천/규칙선택/패턴/검증 계산은 최신 최대 30회만 사용한다.
        ...
        Analysis a;
        if(all.size()<6)a=emptyAnalysis("표본 "+all.size()+"/6 · 최근30 수집중");
        ...
        try{a.tri80=TriPick80Engine.optimize(all);}catch(Throwable e){a.tri80=new TriPick80Engine.Result();a.tri80.detail="삼치기80 30FIX 엔진 보호모드 · "+safeErr(e);}
