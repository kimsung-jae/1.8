...
public class MainActivity extends Activity{
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리 · 삼치기80 BEST1 30FIX",24,Color.WHITE,true));TextView sub=tv("최고 삼치기 1픽만 표시 · 80% 실검증 목표 · PASS 없음",12,Color.rgb(110,231,183),false);sub.setPadding(0,dp(4),0,dp(14));root.addView(sub);
        ...
        LinearLayout hero=card();hero.addView(tv("다음 회차 · 최고 1픽",12,Color.GRAY,false));nextRound=tv("-",15,Color.WHITE,true);hero.addView(nextRound);bestPick=tv("분석 대기",30,Color.rgb(52,211,153),true);bestPick.setPadding(0,dp(7),0,0);hero.addView(bestPick);bestConf=tv("80% 목표 · 최고 삼치기 1픽만 표시",15,Color.rgb(253,224,71),true);bestConf.setPadding(0,dp(4),0,0);hero.addView(bestConf);TextView note=tv("학습데이터 최근 30회 고정 · 31회 이전은 추천 계산에서 제외 · 최고 삼치기 1픽만 표시",12,Color.rgb(110,231,183),true);note.setPadding(0,dp(10),0,0);hero.addView(note);root.addView(hero);
    ...
    private void startExport(){Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"BubbleTriPick80V17_30FIX_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");startActivityForResult(i,REQ_EXPORT);}
    ...
    private void confirmResetStats(){new AlertDialog.Builder(this).setTitle("실전성적 초기화").setMessage("승패·수익 기록만 초기화하고 저장 history는 보존합니다. 분석에는 항상 최신 30회만 사용합니다.").setNegativeButton("취소",null).setPositiveButton("초기화",(d,w)->{FlowCore.resetPerformance(this);Toast.makeText(this,"성적 초기화 완료",Toast.LENGTH_SHORT).show();reloadAsync();}).show();}
