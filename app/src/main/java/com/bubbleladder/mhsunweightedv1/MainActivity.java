...
public class MainActivity extends Activity{
    ...
    private View buildUi(){
        ...
        root.addView(tv("보글사다리 · 삼치기필터 FINAL 1PICK V1.8",24,Color.WHITE,true));TextView sub=tv("1차 삼치기80 필터 → 남은 3개 중 최종 1픽 하나만 추천",12,Color.rgb(110,231,183),false);sub.setPadding(0,dp(4),0,dp(14));root.addView(sub);
        ...
        LinearLayout hero=card();hero.addView(tv("다음 회차 · 최고 1픽",12,Color.GRAY,false));nextRound=tv("-",15,Color.WHITE,true);hero.addView(nextRound);bestPick=tv("분석 대기",30,Color.rgb(52,211,153),true);bestPick.setPadding(0,dp(7),0,0);hero.addView(bestPick);bestConf=tv("최종 1픽 단독검증률을 별도 표시",15,Color.rgb(253,224,71),true);bestConf.setPadding(0,dp(4),0,0);hero.addView(bestConf);TextView note=tv("1차에서 낮은 1개 제외 · 2차에서 Recent/Markov/Shape/KNN 동등투표 · 최종 1개만 표시",12,Color.rgb(110,231,183),true);note.setPadding(0,dp(10),0,0);hero.addView(note);root.addView(hero);
        ...
        LinearLayout bc=card();bc.addView(section("1차 삼치기 + 2차 최종1픽 순차검증"));backtest=tv("-",14,Color.WHITE,false);backtest.setLineSpacing(0,1.3f);bc.addView(backtest);live=tv("실전 기록 · 아직 없음",14,Color.rgb(125,211,252),true);live.setPadding(0,dp(10),0,0);bc.addView(live);root.addView(bc);
        ...
        root.addView(tv("※ 1차 삼치기 80%는 필터 성적이고, 2차 최종 1픽 적중률은 별도입니다. 입력한 배당이 실제 최종 1픽 배당에 적용될 때만 수익 계산이 유효합니다.",11,Color.GRAY,false));return sv;
    }
    ...
    private void manualSync(){
        saveSettingsSilent();refresh.setEnabled(false);status.setText("API 수집 + 삼치기 필터 + 최종 1픽 계산 중...");
    ...
    private void startExport(){Intent i=new Intent(Intent.ACTION_CREATE_DOCUMENT);i.addCategory(Intent.CATEGORY_OPENABLE);i.setType("application/json");i.putExtra(Intent.EXTRA_TITLE,"BubbleFilterBest1V18_"+new SimpleDateFormat("yyyyMMdd_HHmm",Locale.KOREA).format(new Date())+".json");startActivityForResult(i,REQ_EXPORT);}
