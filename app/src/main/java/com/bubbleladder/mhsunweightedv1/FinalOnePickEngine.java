...
public final class FinalOnePickEngine {
    ...
    public static Result optimize(List<FlowCore.Result> all,int currentExcluded){
        ...
        for(int t=from;t<end;t++){
            ...
            int pastEx;
            try{
                TriPick80Engine.Result tr=TriPick80Engine.optimize(prefix);
                pastEx=tr!=null&&valid(tr.excludedCombo)?tr.excludedCombo:fallbackExclude(all,t);
            }catch(Throwable e){
        ...
        r.detail="1차 제외 "+r.excludedLabel+" → 남은 3개 중 최종 "+r.pickLabel+
                ...
                " · 2단계 walk-forward "+r.validationHit+"/"+r.validationN+" = "+pct(r.validationRate);
        return r;
    }
